...
## Import data 
dat <- read_xlsx("processedData/data.xlsx", sheet="threats")
...
df <- dat |> 
  ...
  pivot_longer(cols = starts_with("threat"), 
               names_to = "threat", values_to = "values") |> 
  mutate(
    threat = str_replace(threat , "^threatILK_", "")
  ) |> 
  group_by(threat, values) |> 
  summarise(
    N = n()
  ) |> 
  mutate(
...
# Create the plot with ggpattern
 ggplot(df) +
  aes(x = percent, y = threat, fill = values) +
  geom_bar(
    stat = "identity")+
...
plot_ly(
  ...
  layout(
    ...
    yaxis = list(
      title = list(text= "ILK threats", standoff = 10L),
      titlefont = list(color = "black",size = 16, bold=TRUE),
      tickfont = list(size = 12, color = "black")
    ),
    barmode = 'stack',
